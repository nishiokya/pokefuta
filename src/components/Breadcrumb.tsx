 ...
 *
 * ## 置く条件: 行き先がグローバルナビに無いときだけ
 *
 * 下タブ（SP）と PC ヘッダーは layout から常時描画され、`探す`(/nearby)・
 * `スタンプ帳`(/visits)・`マイ旅`(/my-trip) を常に出している。**そこに載っている
 * 行き先を Breadcrumb でもう一度出すと、同じリンクが画面に2つ並ぶだけ**になる。
 *
 * `/manhole/[id]` は以前 `/nearby` 行きを置いていたが、下タブの「探す」と
 * 完全に同じ行き先だったので外した。現在の利用箇所はいずれも
 * **グローバルナビから辿れない個別ページ**を指している:
 *
 * - `/p/[photoId]` → その写真の `/manhole/[id]`
 * - `/users/[userId]/visits` → `/`
 * - `/users/[userId]/prefectures/[prefecture]` → そのユーザーのスタンプ帳
 *
 * ## router.back() を使わない理由
 *
