 ...
 * 本文先頭の戻る導線。
 *
 * ヘッダー（SiteChrome）は固定内容で戻るボタンを持たない。
 * `/p/[photoId]` のように OGP 経由の共有リンクで深い階層に直接着地する入口では、
 * **ファーストビュー内で唯一の文脈復帰導線**になる。だから必ず本文の最上部に置くこと。
 *
 * （`/p/[photoId]` には本文下部にも同じ `/manhole/[id]` 行きの「このポケふたを見る」が
 * あるが、スクロールしないと出てこないので着地直後の復帰には使えない。）
 *
 * ## 置く条件: 常時見えているタブと同じ行き先を重ねない
 *
 * 下タブ（SP）と PC ヘッダーは layout から描画される。**`探す`(/nearby) と
