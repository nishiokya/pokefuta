...
export default function ManholeDetailPage() {
  ...
  // 1枚ぶんだけ取る。
  //
  // 取得開始時に `loading` / `error` / `derived` を初期化していないのは、
  // **蓋を移ると App Router がこのコンポーネントを作り直すから**。
  // `useState` の初期値に戻るので、前の蓋の内容もエラーも残らない。
  // 本番ビルドで実測（PR #245 のレビュー指摘を検証）:
  //   - 詳細APIを3秒遅らせて関連カードから遷移 → その間ずっと全画面ローディングで、
  //     前の蓋の見出しもバッジも出ない
  //   - 取得を失敗させてエラー画面にしたあと別の蓋へ移ると、エラーは消えて正常描画に戻る
  // つまり**このコンポーネントの正しさは作り直されることに依存している**。
  // ここを共有レイアウト側で使い回す形に変えるなら、初期化を明示的に足すこと。
  //
